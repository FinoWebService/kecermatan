...
document.getElementById('btnGenerateInvite')?.addEventListener('click', async () => {
    ...
    try {
        const result = await window.createAdminInvitation(username, permissions, currentUser.id);
...
async function revokeAdminAction(adminId){
    try {
        await window.revokeAdminAccess(adminId);
...
async function deleteTestAction(testId){
    try {
        await window.deleteTestResult(testId);
